...
public class DomesticFlight extends JFrame {
	...
	public DomesticFlight(LoginPage type1) {
		...
		String[] sItem1 = { "Prayagraj" };
		String[] sItem2 = { "Bengaluru", "Chennai ", "Delhi", "Goa", "Hyderabad", "Kolkata", "Lucknow", "Mumbai",
				"Jaipur" };
		...
		img1 = new ImageIcon("img/ind.jpg");
		...
		BFindFlight.setBounds(60, 270, 100, 25);
		...
		PPanel2.setPreferredSize(new Dimension(315, 160));

		LPassengerDetails = new JLabel("<html><b><font color=\"#C71585\">PassengerDetails</font></b></html>");

		LAdult = new JLabel("Adults(13+)");

		LChildren = new JLabel("Children(2-12)");
		LInfant = new JLabel("Infants(below 2)");
		...
		img2 = new ImageIcon("img/note_bg.gif");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (sClass.equals("Economic")) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Sorry! There is no flight on that day");
				System.exit(0);
			}
		} else {
			try {
		...
		if (iSeatCount > 60) {
			...
			if (iChoice == JOptionPane.YES_OPTION) {
				...
				try {
					// write into data
					Save2 save2 = new Save2(sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate, iPrice,
							sTime);
					ObjectOutputStream OOS1 = new ObjectOutputStream(new FileOutputStream("save2"));
					for (i = 0; i < iCount; i++) {
						Save2 temp1 = new Save2(sTempFrom[i], sTempTo[i], sTempClass[i], iTempAdult[i],
								iTempChildren[i], iTempInfant[i], sTempBookingDate[i], iTempPrice[i], sTempTime[i]);
...
class Save2 implements Serializable {
	...
	public Save2(String sFrom, String sTo, String sClass, Integer iAdult, Integer iChildren, Integer iInfant,
	...
	public String toString() {
		return sFrom + " " + sTo + " " + sClass + " " + iAdult + " " + iChildren + " " + iInfant + " " + sBookingDate
				+ " " + iPrice + " " + sTime;
	}
}