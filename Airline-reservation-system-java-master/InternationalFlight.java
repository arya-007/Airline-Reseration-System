...
public class InternationalFlight extends JFrame {
	...
	public InternationalFlight(LoginPage type1) {
		...
		String[] sItem2 = { "Bali", "Bangkok", "Cairo", "CapeTown", "Chicago", "Dubai", "Frankfurt", "HongKong",
				"Istanbul", "London", "LosAngeles", "Melbourne", "New York", "Paris", "Rome", "SanFrancisco",
				"shanghai", "Singapore", "Sydney", "Toronto" };
		...
		img1 = new ImageIcon("img/map1.jpg");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (iSeatCount > 60) {
			...
			if (iChoice == JOptionPane.YES_OPTION) {
				...
				try {
					// write into data
					Save1 save2 = new Save1(sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate, iPrice,
							sTime);
					ObjectOutputStream OOS1 = new ObjectOutputStream(new FileOutputStream("save1"));
					for (i = 0; i < iCount; i++) {
						Save1 temp1 = new Save1(sTempFrom[i], sTempTo[i], sTempClass[i], iTempAdult[i],
...
class Save1 implements Serializable {
	...
	// int iCount;
	public Save1(String sFrom, String sTo, String sClass, Integer iAdult, Integer iChildren, Integer iInfant,
